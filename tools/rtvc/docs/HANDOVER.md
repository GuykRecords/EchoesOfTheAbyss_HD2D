...
### 2-B. デバイスを繋いだ TOTAL の見込み

```
in-dev 22.00 + block 30.00 + infer 9.28 + xfade 10.00 + out-buf 11.07 + out-dev 24.67
= 107.0 ms
```

**100ms をわずかに超える。** 削る余地は 2 つ：

| 手 | 効果 | 代償 |
|---|---|---|
| `--block-ms 20`（10ms グリッド維持） | **−10ms** | RTF 0.31 → 約 0.46。まだ 0.6 以下 |
| `--prefill-ms` を auto より小さく | 数 ms | ジッタ耐性が減る。`under` を見ながら |

`B=20 / X=10` ならアルゴリズム遅延は 30ms、TOTAL は 95ms 前後の見込み。
