...
### SOLA — ニューラルボコーダーを繋ぐには位置合わせが要る

**`--sola-search-ms` を 0 にしてはいけない。** RVC のボコーダーは窓ごとに
**位相の揃わない波形**を生成する。等パワークロスフェードは「前回の末尾と今回の先頭は
同じ時刻の同じ音」という前提で書かれているが、**その前提が成り立たない。**

実測（位相をランダムにする擬似ボコーダー、200Hz）:

| 探索幅 S | 継ぎ目での振幅 |
|---|---|
| 0ms（SOLA なし） | 期待値の **0.64**（36% 打ち消し） |
| 5ms 以上 | 期待値の **1.00**（完全に一致） |

毎回いちばん合う位置を相互相関で探してから繋ぐ。これが SOLA で、RVC 本家の
リアルタイム実装も同じことをしている。

**S は出力される声の基本周期より長く取ること。**

| 声 | 基本周波数の目安 | 必要な S |
|---|---|---|
| 高い声（つくよみちゃん等） | 200〜250Hz | 5ms で足りる。既定 10ms |
| 低い男声 | 85〜120Hz | **20ms** 必要 |

**S はそのまま遅延になる**（`B + X + S`）。10ms グリッドを守ること。
...
- 窓を明示しなければ **30/10/100/S=10** に自動設定される
...
## 7-A. 窓の大きさ — 実測（2026-09-02、つくよみちゃん公式モデル）

**旧メモの `B=30 / X=10 / EXTRA=100` では言葉として成立しない。**
あの値は「アルゴリズム遅延を 40ms に揃える」ために選ばれたもので、
RVC がその窓で動くかは確かめられていなかった。

同一音声を録って（`--record-in`）、設定だけ変えて聴き比べた結果：

| 設定 | ぶつぶつ | 溶ける |
|---|---|---|
| `B=30 X=10` | あり | あり（**何を言っているか分からない**） |
| `B=70 X=10` | **あり** | あり |
| `B=30 X=50` | なし | あり |
| `B=60 X=50` | なし | **改善（実用域）** |
| `B=130 X=50` | なし | 良い |
| `B=250 X=50` | なし | 良い（130 と同等） |

**2 つの軸は別々の症状に対応する。**

| 軸 | 効くもの | 実用下限 |
|---|---|---|
| **B（ブロック）** | 音の**明瞭さ**。小さいと溶ける | **60ms** |
| **X（クロスフェード）** | 継ぎ目の**滑らかさ**。小さいとぶつぶつ | **50ms** |
| EXTRA | 文脈。**遅延に効かない** | 2500ms で運用（安い） |

B が効くのは、RVC のリアルタイム経路が f0 のキャッシュを
「1 回の呼び出しでこれくらい進む」前提で持っているため（本家の既定ブロックは 250ms 前後）。

X が 50ms 要るのは、SOLA で位置を合わせてもボコーダーの出力が完全には
一致しないため。長いクロスフェードが残差を隠している。

### ノイズゲートは RVC の前に置かない

**入力のノイズゲートが語尾を削っていた。** ゲートは `-50dBFS` で閉じるが、
**語尾は音量が下がる**ので、そこで閉じ始めると RVC に渡る前に語尾が失われる。
同じ音声で `--no-gate` と比較して、明らかにこちらが良い。

環境音を少し減らす代わりに語尾を失うのは割に合わない。**`--engine rvc` では既定で off。**
`--gate` で明示的に有効化はできる。
...
### 落ち着いた設定（`--engine rvc` の既定）

```
B=130ms  X=50ms  EXTRA=2500ms  S=10ms  gate=off
アルゴリズム遅延 = 130 + 50 + 10 = 190ms
```

**旧メモの「B+X=40ms」前提は成立しない。**
既定値は「遅延を揃えるために選んだ値」ではなく、**実際に聴いて決めた値**に置き換えてある。
