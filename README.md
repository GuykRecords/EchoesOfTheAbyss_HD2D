...
| `scripts/prepare_dataset.py` | 既存の音源を学習用の 48k モノラル WAV に切り分ける（入力は変更しない） |
| `scripts/check_dataset.py` | 学習用の音声が使い物になるかの判定（読み取り専用） |
...
184 件。音声デバイスも GPU も不要。GitHub Actions で push のたびに自動実行される
...
もともとこのリポジトリは HD-2D ゲーム `EchoesOfTheAbyss_HD2D` として作られ、
音声ツールはその `tools/rtvc/` に間借りしていた。ゲーム側は着手されないまま
音声側が主題になったので、2026-09-02 に入れ替えた。

**Unity プロジェクトは `unity-hd2d` ブランチにそのまま残してある。**
テンプレートのままだが、再開するときはそこから取り出せる。

```bash
git checkout unity-hd2d
```
