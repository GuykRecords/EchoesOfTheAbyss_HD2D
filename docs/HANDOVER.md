...
### RVC 純正のリアルタイム GUI でも同じ症状が出た

`realtime_gui.py` を動かしたところ、**同じく音程が揺れ、変換も浅い**。
（起動時に `assets/weights/kikiV1.pth` が無いと落ちる。`configs/config.json` が
前回の設定を保持しているため。退避してから起動すること）

**私たちのラッパーが原因ではない。** 純正も同じ `infer/rtrvc.py` を使っており、
差が出るのは常にファイル変換（`pipeline.py`）との間。**リアルタイム経路そのものが
天井**で、窓を直しても越えられない。

これで切り分けは終わり。残る差の候補は 2 つに絞られた:

1. **`protect`（子音・呼吸音の保護）** — `pipeline.py` にあり、`rtrvc.py` に無い
2. **ブロックごとの f0 再取得** — 「うにょうにょ揺れる」「ケロケロ」の正体

どちらも直すには `rtrvc.RVC.infer` の中身に手を入れることになる。
**RVC 本体は書き換えず、`rvc_backend.py` 側から差し替える形にすること。**
